...
function createBucket(options) {
    ...
    function _createBucket(name, cfg, opts, res) {
        ...
        if (options.ring_.ro_) {
            return res.end(new Error('unable to create bucket, some vnodes ' +
...
function updateBucket(options) {
    ...
    function _updateBucket(name, cfg, opts, res) {
        ...
        if (options.ring_.ro_) {
            return res.end(new Error('unable to update bucket, some vnodes ' +
                                     'are in read-only mode'));
        }
...
function delBucket(options) {
    ...
    function _delBucket(name, opts, res) {
        ...
        if (options.ring_.ro_) {
            return res.end(new Error('unable to delete bucket, some vnodes ' +
                                     'are in read-only mode'));
        }
...
function putObject(options) {
    ...
    function _putObject(b, k, v, opts, res) {
        ...
        if (node.data && node.data === READ_ONLY) {
            return res.end(new Error('unable to modify object, vnode is in ' +
                                     'read-only mode'));
        }
        v.vnode = node.vnode;
