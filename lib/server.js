...
function createServer(options) {
    ...
    assert.string(options.ring, 'options.ring');
    ...
    ring.deserializeRing({
        ...
        createClient({
            ...
            if (err) {
                throw new verror.VError(err, 'unable to create moray clients');
            }
...
function getBucket(options) {
    ...
    function _getBucket(opts, bucket, res) {
        ...
        //var pnode = options.ring.getNode(uuid(), uuid(), function(err, hNode) {
...
function putObject(options) {
    ...
    function _putObject(b, k, v, opts, res) {
        ...
        options.ring.getNode(b, k, function (err, node) {
            ...
            client.putObject(b, k, v, opts, function (err, meta) {
                log.debug({
                    err: err,
                    meta: meta
                }, 'putObject: returned');
...
function getObject(options) {
    ...
    function _getObject(b, k, opts, res) {
        ...
        options.ring.getNode(b, k, function (err, node) {
            ...
            var pnode = node.pnode;
            var client = options.clients.map[pnode];
            client.getObject(b, k, opts, function (err, obj) {
                log.debug({
                    err: err,
                    obj: obj
                }, 'getObject: returned');
...
function delObject(options) {
    ...
    function _delObject(b, k, opts, res) {
        ...
        //var node = options.ring.getNode(b, k, function (err, node) {
        options.ring.getNode(b, k, function (err, node) {
            if (err) {
                res.end(err);
                return (undefined);
            ...
            client.delObject(b, k, opts, function (err) {
                log.debug({
                    err: err
                }, 'delObject: returned');
